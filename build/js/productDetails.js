var o = (a, s, t) =>
  new Promise((e, r) => {
    var u = (d) => {
      ...
      i = (d) => (d.done ? e(d.value) : Promise.resolve(d.value).then(u, l));
    i((t = t.apply(a, s)).next());
  });
export default class p {
  constructor(s, t) {
    (this.productId = s), (this.product = {}), (this.dataSource = t);
  }
  init() {
    return o(this, null, function* () {
      (this.product = yield this.dataSource.findProductById(this.productId)),
        ...
        document
          ...
          .addEventListener("click", this.addToCart.bind(this.productId));
      const s = document.querySelectorAll("#addToCart");
      s.forEach((t) => {
        t.addEventListener("click", (e) => {
          if ((e.preventDefault(), !t.classList.contains("add"))) {
            t.classList.add("add");
            const r = document.querySelector(".button-added");
            r.setAttribute("class", "button-add"),
              setTimeout(() => {
                t.classList.remove("add"),
                  r.classList.remove("button-add"),
                  r.setAttribute("class", "button-added");
              }, 3e3);
          }
        });
      });
